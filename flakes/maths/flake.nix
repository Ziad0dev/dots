    ...
    in
    {
      packages = each (pkgs:
        let
          ...
          report = pkgs.writeText "maths-missing" (lib.concatStringsSep "\n"
            (lib.mapAttrsToList
      ...
      devShells = each (pkgs:
        ...
        lib.mapAttrs shell p.sets // {
          ...
          mathlib = pkgs.mkShellNoCC {
            name = "maths-mathlib";
            packages = p.sets.lean ++ [ pkgs.git pkgs.curl pkgs.cacert ];
            LD_LIBRARY_PATH = lib.makeLibraryPath [ pkgs.stdenv.cc.cc.lib pkgs.gmp pkgs.zlib ];
          };
        });
