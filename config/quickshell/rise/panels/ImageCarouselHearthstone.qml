...
PanelWindow {
    ...
    WlrLayershell.namespace: "dots-image-carousel-hs"
    ...
    function buildScanCmd() {
        if (isThemeMode) {
            return ["bash", "-c", "D=$HOME/.cache/quickshell-img-thumbs; mkdir -p \"$D\"; HASHCACHE=$HOME/.cache/quickshell-img-thumb-hashes.tsv; touch \"$HASHCACHE\"; TL=$(mktemp); TK=$(mktemp); TM=$(mktemp); TD=$(mktemp); TW=$(mktemp); trap 'rm -f \"$TL\" \"$TK\" \"$TM\" \"$TD\" \"$TW\" \"$TK.hit\" \"$TM.sha\"' EXIT; thumbs_for_list() { : > \"$TK.hit\"; [ -s \"$TL\" ] || return 0; paste \"$TL\" <(xargs -r -d '\\n' -a \"$TL\" readlink -f --) <(xargs -r -d '\\n' -a \"$TL\" stat -Lc '%s:%Y:%Z' --) > \"$TK\"; : > \"$TM\"; awk -F '\\t' -v HC=\"$HASHCACHE\" -v MISS=\"$TM\" 'BEGIN { while ((getline l < HC) > 0) { i = index(l, \"\\t\"); if (i) { v = substr(l, i+1); if (v ~ /^[0-9a-f]{64}$/) h[substr(l, 1, i-1)] = v } } close(HC) } { k = $2 \"|\" $3; if (k in h) print $1 \"\\t\" h[k]; else print $1 \"\\t\" k > MISS }' \"$TK\" > \"$TK.hit\"; [ -s \"$TM\" ] || return 0; cut -f1 \"$TM\" | xargs -r -d '\\n' sha256sum -- 2>/dev/null | sed 's/^\\([0-9a-f]*\\)  /\\1\\t/' | awk -F '\\t' '{ s[$2] = $1 } END { for (p in s) print p \"\\t\" s[p] }' > \"$TM.sha\"; awk -F '\\t' 'NR == FNR { s[$1] = $2; next } ($1 in s) { print $2 \"\\t\" s[$1] }' \"$TM.sha\" \"$TM\" | flock \"$HASHCACHE\" tee -a \"$HASHCACHE\" >/dev/null; awk -F '\\t' 'NR == FNR { s[$1] = $2; next } ($1 in s) { print $1 \"\\t\" s[$1] }' \"$TM.sha\" \"$TM\" >> \"$TK.hit\"; }; CACHE=$HOME/.cache/quickshell-theme-picker; mkdir -p \"$CACHE\"; shopt -s nullglob nocaseglob; : > \"$TL\"; : > \"$TD\"; for d in ~/dots/config/themes/*; do [ -d \"$d\" ] || continue; name=$(basename \"$d\"); prev=\"\"; for c in \"$d\"/preview.png \"$d\"/preview.jpg \"$d\"/preview.jpeg; do [ -f \"$c\" ] && { prev=\"$c\"; break; }; done; if [ -z \"$prev\" ]; then bgs=(\"$d\"/backgrounds/*.jpg \"$d\"/backgrounds/*.jpeg \"$d\"/backgrounds/*.png \"$d\"/backgrounds/*.webp); prev=\"${bgs[0]}\"; fi; [ -z \"$prev\" ] && continue; ext=\"${prev##*.}\"; link=\"$CACHE/$name.$ext\"; [ -L \"$link\" ] || ln -sf \"$prev\" \"$link\"; printf '%s\\n' \"$link\" >> \"$TL\"; printf '%s\\t%s\\n' \"$link\" \"$d\" >> \"$TD\"; done; thumbs_for_list; awk -F '\\t' -v D=\"$D\" 'NR == FNR { dir[$1] = $2; next } $2 != \"\" { print $1 \"\\t\" D \"/\" $2 \"-512.jpg\" \"\\t\" dir[$1] }' \"$TD\" \"$TK.hit\" | sort -u"]
        } else {
            return ["bash", "-c", "D=$HOME/.cache/quickshell-img-thumbs; mkdir -p \"$D\"; HASHCACHE=$HOME/.cache/quickshell-img-thumb-hashes.tsv; touch \"$HASHCACHE\"; TL=$(mktemp); TK=$(mktemp); TM=$(mktemp); TD=$(mktemp); TW=$(mktemp); trap 'rm -f \"$TL\" \"$TK\" \"$TM\" \"$TD\" \"$TW\" \"$TK.hit\" \"$TM.sha\"' EXIT; thumbs_for_list() { : > \"$TK.hit\"; [ -s \"$TL\" ] || return 0; paste \"$TL\" <(xargs -r -d '\\n' -a \"$TL\" readlink -f --) <(xargs -r -d '\\n' -a \"$TL\" stat -Lc '%s:%Y:%Z' --) > \"$TK\"; : > \"$TM\"; awk -F '\\t' -v HC=\"$HASHCACHE\" -v MISS=\"$TM\" 'BEGIN { while ((getline l < HC) > 0) { i = index(l, \"\\t\"); if (i) { v = substr(l, i+1); if (v ~ /^[0-9a-f]{64}$/) h[substr(l, 1, i-1)] = v } } close(HC) } { k = $2 \"|\" $3; if (k in h) print $1 \"\\t\" h[k]; else print $1 \"\\t\" k > MISS }' \"$TK\" > \"$TK.hit\"; [ -s \"$TM\" ] || return 0; cut -f1 \"$TM\" | xargs -r -d '\\n' sha256sum -- 2>/dev/null | sed 's/^\\([0-9a-f]*\\)  /\\1\\t/' | awk -F '\\t' '{ s[$2] = $1 } END { for (p in s) print p \"\\t\" s[p] }' > \"$TM.sha\"; awk -F '\\t' 'NR == FNR { s[$1] = $2; next } ($1 in s) { print $2 \"\\t\" s[$1] }' \"$TM.sha\" \"$TM\" | flock \"$HASHCACHE\" tee -a \"$HASHCACHE\" >/dev/null; awk -F '\\t' 'NR == FNR { s[$1] = $2; next } ($1 in s) { print $1 \"\\t\" s[$1] }' \"$TM.sha\" \"$TM\" >> \"$TK.hit\"; }; find -L " + wallpaperFindRoots() + " -maxdepth 2 -type f \\( -iname '*.jpg' -o -iname '*.jpeg' -o -iname '*.png' -o -iname '*.webp' -o -iname '*.bmp' -o -iname '*.gif' \\) 2>/dev/null | sort > \"$TL\"; thumbs_for_list; sort \"$TK.hit\" | awk -F '\\t' -v D=\"$D\" '$2 != \"\" { print $1 \"\\t\" D \"/\" $2 \"-512.jpg\" }'"]
        }
    }
