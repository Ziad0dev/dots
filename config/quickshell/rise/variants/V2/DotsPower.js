...
var dotsPathCmd = "PATH=\"$HOME/.nix-profile/bin:$PATH\"; export PATH; "

// Prefer dots-shell's display resolver when present. The fallback matches
// dots-brightness-display's device preference, with /sys-anchored globs so
// Quickshell's working directory cannot affect the selected device.
var backlightDeviceCmd = dotsPathCmd + "if command -v dots-hw-display >/dev/null 2>&1; then BL=$(dots-hw-display 2>/dev/null) || exit 0; [ -n \"$BL\" ] || exit 0; else BL=$(ls -1 /sys/class/backlight 2>/dev/null | head -n1); for C in /sys/class/backlight/amdgpu_bl* /sys/class/backlight/intel_backlight /sys/class/backlight/acpi_video*; do [ -e \"$C\" ] && { BL=\"${C##*/}\"; break; }; done; [ -n \"$BL\" ] || exit 0; fi; "
...
function brightnessSetCmd(step) {
    ...
    return dotsPathCmd +
        "if command -v dots-brightness-display >/dev/null 2>&1; then " +
        "dots-brightness-display " + qStep + " >/dev/null 2>&1; " +
        "else " +
        "RUNTIME_DIR=${XDG_RUNTIME_DIR:-/tmp}; exec 9>\"$RUNTIME_DIR/dots-brightness-display.lock\"; flock -n 9 || exit 0; " +
        backlightDeviceCmd +
        "STEP=" + qStep + "; " +
        ...
        "if command -v dots-swayosd-brightness >/dev/null 2>&1; then dots-swayosd-brightness \"$(brightnessctl -d \"$BL\" -m 2>/dev/null | cut -d',' -f4 | tr -d '%')\" >/dev/null 2>&1; fi; " +
        "fi"
}
