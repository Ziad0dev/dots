...
PanelWindow {
    ...
    WlrLayershell.namespace: "dots-image-picker-hs"
    ...
    Process {
        ...
        command: panel.isThemeMode
            ...
            : ["bash", "-c", "dots-current-wallpaper 2>/dev/null || true"]
        running: false
    ...
    function buildScanCmd() {
        if (isThemeMode) {
            return ["bash", "-c",
                "dots-theme-scan " + panel.themeFindRoots()]
        } else {
            return ["bash", "-c", [
    ...
    function applySelected() {
        ...
        if (isThemeMode) {
            ...
            applyThemeProc.command = ["themectl", "set", name]
            applyThemeProc.running = false; applyThemeProc.running = true
        } else {
            applyBgProc.command = ["dots-set-wallpaper", path]
    ...
    function themeFindRoots() {
        var paths = root.themeSourcePaths || []
        var args = []
